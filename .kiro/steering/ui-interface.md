...
- **Components**:
  ...
  - Tooltips on input labels showing parameter descriptions
  - Form validation and error handling
  - Visualize button (submits form)
...
- **Components**:
  ...
  - Tooltips showing part names on hover
...
- `x-progress-bar` - Progress bar with percentage
- `x-navigation-rail` - Assembly navigation
- `x-navigation-rail-button` - Navigation rail button
- `x-tab-panel` - Tab content container
- `x-container` - Content container wrapper
- `x-inputs-form` - Parameter input form
- `x-download-button` - Download button component
- `x-header` - Application header
- `x-typography` - Typography component
- `x-wind-turbine-icon` - Wind turbine icon
