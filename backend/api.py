...
def execute_operation(
    ...
    request_body: dict,
    path_variables: dict,
    invoke_operation: Callable,
) -> None:
    """Execute operation in separate process."""
    import os
    import sys
    import traceback
    from datetime import datetime, timezone

    date_time = datetime.now(timezone.utc).strftime("%d/%b/%Y %H:%M:%S")
    sys.stderr.write(
        f"{date_time} [PID {os.getpid()}] [PPID {os.getppid()}] {operation.__name__}\n"
    )
    try:
        request = {"body": request_body, "path": path_variables}
        ...
        value = exception
        has_exception = True
    connection.send({"value": value, "has_exception": has_exception})
    connection.close()
...
class Api:
    ...
    def run(self, host, port):
        Handler = create_request_handler(self._operations_by_method_and_path, root_path)
...
def create_request_handler(
    operations_by_method_and_path: Dict[str, Callable], directory: Path
    ...
    class Handler(SimpleHTTPRequestHandler):
        ...
        def handle_api_request(self):
            for method_and_path, operation in operations_by_method_and_path.items():
                ...
                path_variable_match = re.search(path_variable_pattern, path)
                ...
                if path_variable_match:
                    ...
                    path_pattern = re.compile(
                        path.replace(path_variable_match.group(0), "([A-Za-z0-9-_]+)")
                    )
...
def invoke_operation(
    operation: Callable, request_body: Optional[dict]
) -> Union[dict, bytes]:
...
def load_furl_transform_from_parameters(parameters: dict) -> List[dict]:
    ...
    return load_furl_transform(magnafpm_parameters, user_parameters, furling_parameters)
...
def visualize_from_parameters(parameters: dict, assembly) -> str:
    ...
    return assembly_to_obj(
        assembly, magnafpm_parameters, user_parameters, furling_parameters
    )
...
def handle_create_archive(request: dict) -> bytes:
    ...
    return create_archive(magnafpm_parameters, user_parameters, furling_parameters)
...
def handle_create_dxf_archive(request: dict) -> bytes:
    ...
    return export_to_dxf(magnafpm_parameters, user_parameters, furling_parameters)
